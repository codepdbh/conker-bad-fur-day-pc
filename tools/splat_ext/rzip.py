...
try:
    ...
    from splat.util.options import opts
except ImportError:
    from src.splat.segtypes.segment import Segment
    from src.splat.util.options import opts

import sys
ext_path_str = str(opts.extensions_path) if opts.extensions_path else 'tools/splat_ext'
if ext_path_str not in sys.path:
    sys.path.append(ext_path_str)
if 'tools/splat_ext' not in sys.path:
    sys.path.append('tools/splat_ext')
import rareunzip

class N64SegRzip(Segment):
    def __init__(self, rom_start, rom_end, type, name, vram_start, args=None, yaml=None, **kwargs):
        super().__init__(rom_start, rom_end, type, name, vram_start, args=args, yaml=yaml, **kwargs)
        self.has_subsegments = "subsegments" in (yaml or {})
        self.yaml = yaml or {}
        self.xor = self.yaml.get("xor", None)
