...
from ..util.metadata.segment_manager import manager
...
class Segment:
    ...
    def __init__(
        ...
        self.prioritized_segments: list[str] = []
    ...
    def from_yaml(
        ...
        if (
            ...
            if isinstance(parent, CommonSegCode):
                ...
                if isinstance(ret.vram_start, int) and isinstance(parent.vram_end, int):
                    # Check if this segment starts after the end of the parent.
                    # Allow this segment to start at the end of the parent only if it is zero sized.
                    if ret.vram_start > parent.vram_end or (
                        ret.vram_start == parent.vram_end
                        and ret.vram_end != ret.vram_start
                    ):
                        log.error(
    ...
    def get_symbol(
        ...
        if parent_segment_info is None:
            ...
            seg_meta = manager.unknown_segment
            ...
            if most_parent.owned_metadata is not None:
                ...
                # Try to retrieve it.
                seg_meta = manager.find_owned_segment(parent_segment_info)
            ret = seg_meta.find_symbol(addr, search_ranges)
        elif not local_only:
            # Try to find the symbol anywhere.
            ret = manager.find_symbol_from_any_segment(
                addr,
                parent_segment_info,
                search_ranges,
                validation,
    ...
    def create_symbol(
        ...
        if parent_segment_info is None:
            ...
            seg_meta = manager.find_referenced_segment_for_creation(
                addr,
                parent_segment_info,
            )
