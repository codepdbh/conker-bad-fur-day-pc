...
bool N64Recomp::analyze_function(const N64Recomp::Context& context, const N64Recomp::Function& func,
    ...
    for (size_t i = 0; i < stats.jump_tables.size(); i++) {
        ...
        // Find which section contains the jump table:
        uint32_t jtbl_section_index = func.section_index;
        uint32_t jtbl_base_rom = func.rom;
        uint32_t jtbl_base_vram = func.vram;

        for (size_t sec_idx = 0; sec_idx < context.sections.size(); sec_idx++) {
            const auto& sec = context.sections[sec_idx];
            if (cur_jtbl.vram >= sec.ram_addr && cur_jtbl.vram < sec.ram_addr + sec.size) {
                jtbl_section_index = (uint32_t)sec_idx;
                jtbl_base_rom = sec.rom_addr;
                jtbl_base_vram = sec.ram_addr;
                break;
            }
        }

        cur_jtbl.rom = cur_jtbl.vram - jtbl_base_vram + jtbl_base_rom;
        cur_jtbl.section_index = jtbl_section_index;

        while (vram < end_address) {
            // Retrieve the current entry of the jump table
            uint32_t rom_addr = vram - jtbl_base_vram + jtbl_base_rom;
            if (rom_addr + 4 > context.rom.size()) {
                break;
            }
        ...
        if (cur_jtbl.entries.size() == 0) {
            // Not a real jump table, false positive from heuristic - treat as indirect jump/return
