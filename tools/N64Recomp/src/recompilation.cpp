...
bool process_instruction(GeneratorType& generator, const N64Recomp::Context& context, const N64Recomp::Function& func, size_t func_index, const N64Recomp::FunctionStats& stats, const std::unordered_set<uint32_t>& jtbl_lw_instructions, size_t instr_index, const std::vector<rabbitizer::InstructionCpu>& instructions, std::ostream& output_file, bool indent, bool emit_link_branch, int link_branch_index, size_t reloc_index, bool& needs_link_branch, bool& is_branch_likely, bool tag_reference_relocs, std::span<std::vector<uint32_t>> static_funcs_out) {
    ...
    auto print_branch = [&](uint32_t branch_target) {
        // If the branch target is outside the current function, treat it as a tail call.
    ...
    case InstrId::cpu_mfc0:
        {
            ...
            case Cop0Reg::COP0_Status:
                ...
                break;
            case Cop0Reg::COP0_Cause:
                print_indent();
                generator.emit_cop0_cause_read(rt);
                break;
            case Cop0Reg::COP0_Count:
                print_indent();
                generator.emit_cop0_count_read(rt);
                break;
            case Cop0Reg::COP0_Compare:
                print_indent();
                generator.emit_cop0_compare_read(rt);
                break;
            case Cop0Reg::COP0_EPC:
            case Cop0Reg::COP0_ErrorEPC:
                print_indent();
                generator.emit_cop0_epc_read(rt);
                break;
            case Cop0Reg::COP0_BadVaddr:
                print_indent();
                generator.emit_cop0_badvaddr_read(rt);
                break;
            default:
                print_indent();
                generator.emit_cop0_generic_read(rt, (int)reg);
                break;
            }
            break;
        }
    case InstrId::cpu_mtc0:
        {
            ...
            case Cop0Reg::COP0_Cause:
                print_indent();
                generator.emit_cop0_cause_write(rt);
                break;
            case Cop0Reg::COP0_Count:
                print_indent();
                generator.emit_cop0_count_write(rt);
                break;
            case Cop0Reg::COP0_Compare:
                print_indent();
                generator.emit_cop0_compare_write(rt);
                break;
            default:
                // Hardware cache/TLB initializations in OS boot - no-op on modern PC
                print_indent();
                generator.emit_comment(fmt::format("mtc0 cop0 reg {}", (int)reg));
                break;
            }
            break;
        }
    ...
    case InstrId::cpu_jalr:
        if (rd != (int)rabbitizer::Registers::Cpu::GprO32::GPR_O32_ra) {
            print_indent();
            fmt::print(output_file, "ctx->r{} = S32(0x{:08X});\n", rd, instr_vram + 8);
        } else {
