...
#define MEM_W(offset, reg) \
    (*(int32_t*)(rdram + RECOMP_PHYS_ADDR((reg) + (offset))))

#define MEM_H(offset, reg) \
    (*(int16_t*)(rdram + (RECOMP_PHYS_ADDR((reg) + (offset)) ^ 2)))

#define MEM_B(offset, reg) \
    (*(int8_t*)(rdram + (RECOMP_PHYS_ADDR((reg) + (offset)) ^ 3)))

#define MEM_HU(offset, reg) \
    (*(uint16_t*)(rdram + (RECOMP_PHYS_ADDR((reg) + (offset)) ^ 2)))

#define MEM_BU(offset, reg) \
    (*(uint8_t*)(rdram + (RECOMP_PHYS_ADDR((reg) + (offset)) ^ 3)))

#define SD(val, offset, reg) { \
    *(uint32_t*)(rdram + RECOMP_PHYS_ADDR((reg) + (offset) + 4)) = (uint32_t)((gpr)(val) >> 0); \
    *(uint32_t*)(rdram + RECOMP_PHYS_ADDR((reg) + (offset) + 0)) = (uint32_t)((gpr)(val) >> 32); \
}
