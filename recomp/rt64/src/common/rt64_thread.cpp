...
#   if defined(_WIN32)
        ...
        static auto SetThreadDescriptionFunc = (pSetThreadDescription)GetProcAddress(GetModuleHandleA("kernel32.dll"), "SetThreadDescription");
        if (SetThreadDescriptionFunc) {
            std::wstring nameWide = win32::Utf8ToUtf16(str);
            SetThreadDescriptionFunc(GetCurrentThread(), nameWide.c_str());
        }
#   elif defined(__linux__)
