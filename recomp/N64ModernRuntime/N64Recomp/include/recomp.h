...
extern uint8_t g_mmio_dummy[0x10000];
#ifdef __cplusplus
}
#endif
...
static inline uint8_t* get_phys_ptr(uint8_t* rdram, uint32_t addr) {
    ...
    if (phys >= 0x10000000 && phys < 0x14000000) {
        if (g_rom_data != NULL) {
            uint32_t rom_offset = (phys - 0x10000000) & (uint32_t)(g_rom_size - 1);
            return g_rom_data + rom_offset;
        }
    }
    return rdram + (phys & 0x00FFFFFF);
}

#define RECOMP_PHYS_ADDR(addr) (((uint32_t)(uintptr_t)(addr)) & 0x00FFFFFF)

#define MEM_W(offset, reg) \
    (*(int32_t*)(get_phys_ptr(rdram, (uint32_t)((reg) + (offset)))))

#define MEM_H(offset, reg) \
    (*(int16_t*)(get_phys_ptr(rdram, ((uint32_t)((reg) + (offset))) ^ 2)))

#define MEM_B(offset, reg) \
    (*(int8_t*)(get_phys_ptr(rdram, ((uint32_t)((reg) + (offset))) ^ 3)))

#define MEM_HU(offset, reg) \
    (*(uint16_t*)(get_phys_ptr(rdram, ((uint32_t)((reg) + (offset))) ^ 2)))

#define MEM_BU(offset, reg) \
    (*(uint8_t*)(get_phys_ptr(rdram, ((uint32_t)((reg) + (offset))) ^ 3)))

#define SD(val, offset, reg) { \
    *(uint32_t*)(get_phys_ptr(rdram, (uint32_t)((reg) + (offset) + 4))) = (uint32_t)((gpr)(val) >> 0); \
    *(uint32_t*)(get_phys_ptr(rdram, (uint32_t)((reg) + (offset) + 0))) = (uint32_t)((gpr)(val) >> 32); \
}
