...
extern const OSViMode dummy_mode;
...
void vi_thread_func() {
    try {
        ...
        fprintf(stderr, "[VI Thread Exception caught]\n");
    }
}
...
void gfx_thread_func(uint8_t* rdram, moodycamel::LightweightSemaphore* thread_ready, ultramodern::renderer::WindowHandle window_handle) {
    ...
    try {
        ...
        if (renderer_context == nullptr) {
            fprintf(stderr, "[Error] create_render_context returned nullptr!\n");
    ...
    renderer_context->shutdown();
        ...
        fprintf(stderr, "[GfxThread Exception]: %s\n", e.what());
        thread_ready->signal();
    } catch (...) {
        fprintf(stderr, "[GfxThread Unknown Exception]\n");
        thread_ready->signal();
    }
}
...
static void dump_framebuffer_bmp(uint8_t* rdram, uint32_t fb_addr, int w, int h, const char* path) {
    if (!rdram || fb_addr == 0) return;
    uint32_t phys = fb_addr & 0x00FFFFFF;
    if (phys + (uint32_t)(w * h * 2) > 16 * 1024 * 1024) return;
    uint16_t* src = (uint16_t*)&rdram[phys];
    ...
    uint32_t filesize = 54 + row_padded * h;
    *(uint32_t*)&bmpfileheader[2] = filesize;
    *(int32_t*)&bmpinfoheader[4] = w;
    *(int32_t*)&bmpinfoheader[8] = h;
    *(uint32_t*)&bmpinfoheader[20] = row_padded * h;
...
extern "C" void osViSwapBuffer(RDRAM_ARG PTR(void) frameBufPtr) {
    ...
    swap_count++;
    if (swap_count % 30 == 1) {
        fprintf(stdout, "[Conker VI] Active Frame Swap #%d: FB=0x%08X\n", swap_count, (uint32_t)(uintptr_t)frameBufPtr);
        fflush(stdout);
        dump_framebuffer_bmp(rdram, (uint32_t)(uintptr_t)frameBufPtr, 320, 240, "conker_gameplay.bmp");
    }
    std::lock_guard lock{ events_context.message_mutex };
...
extern "C" void osViSetMode(RDRAM_ARG PTR(OSViMode) mode_) {
    ...
    ViState* next_state = events_context.vi.get_next_state();
    next_state->mode = mode ? mode : &dummy_mode;
    next_state->control = next_state->mode->comRegs.ctrl;
}
...
void ultramodern::init_events(RDRAM_ARG ultramodern::renderer::WindowHandle window_handle) {
    ...
    if (setup_result != ultramodern::renderer::SetupResult::Success) {
        fprintf(stderr, "[Conker] Renderer setup returned status %d. Continuing in compatibility mode.\n", (int)setup_result);
    }
