...
void ultramodern::thread_queue_insert(RDRAM_ARG PTR(PTR(OSThread)) queue_, PTR(OSThread) toadd_) {
    ...
    fprintf(stdout, "[Thread Queue] Inserting thread %d (pri %d) into queue 0x%08X\n", toadd->id, toadd->priority, (uintptr_t)queue_);
...
PTR(OSThread) ultramodern::thread_queue_pop(RDRAM_ARG PTR(PTR(OSThread)) queue_) {
    ...
    *queue = TO_PTR(OSThread, ret)->next;
    ...
    fprintf(stdout, "[Thread Queue] Popped thread %d (pri %d) from queue 0x%08X (caller=%p)\n", TO_PTR(OSThread, ret)->id, TO_PTR(OSThread, ret)->priority, (uintptr_t)queue_, __builtin_return_address(0));
    fflush(stdout);
    return ret;
}
