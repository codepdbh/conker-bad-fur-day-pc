...
void run_thread_function(uint8_t* rdram, uint64_t addr, uint64_t sp, uint64_t arg) {
    fprintf(stdout, "[Conker Thread] run_thread_function: addr=0x%llX, sp=0x%llX, arg=0x%llX\n", (unsigned long long)addr, (unsigned long long)sp, (unsigned long long)arg);
    fflush(stdout);
    ...
    if (current_game.has_value()) {
        ...
        if (find_it != game_roms.end() && find_it->second.thread_create_callback != nullptr) {
            find_it->second.thread_create_callback(rdram, &ctx);
        }
    }

    recomp_func_t* func = get_function(addr);
    fprintf(stdout, "[Conker Thread] get_function(0x%llX) = %p\n", (unsigned long long)addr, (void*)func);
    fflush(stdout);
    if (func != nullptr) {
        func(rdram, &ctx);
    } else {
        fprintf(stderr, "[Conker Thread Error] Function not found for addr 0x%llX!\n", (unsigned long long)addr);
        fflush(stderr);
    }
}
...
bool recomp::wait_for_game_started(uint8_t* rdram, recomp_context* context) {
    fprintf(stdout, "[Conker] wait_for_game_started entered, waiting on game_status...\n");
    fflush(stdout);
    game_status.wait(GameStatus::None);

    fprintf(stdout, "[Conker] wait_for_game_started: game_status is %d\n", (int)game_status.load());
    fflush(stdout);
    ...
    switch (game_status.load()) {
        ...
        case GameStatus::Running:
            {
                fprintf(stdout, "[Conker] GameStatus::Running active. Initializing overlays and entrypoint...\n");
                fflush(stdout);
                ...
                recomp::GameEntry game_entry;
                {
                    ...
                    auto find_it = game_roms.find(current_game.value());
                    if (find_it == game_roms.end()) {
                        fprintf(stderr, "[Conker] Error: game_roms entry not found for game_id!\n");
                        fflush(stderr);
                        return false;
                    }
                    game_entry = find_it->second;
                }

                fprintf(stdout, "[Conker] Loading overlays and initial DMA for %s...\n", game_entry.display_name.c_str());
                fflush(stdout);
                ...
                fprintf(stdout, "[Conker] Invoking recompiled entrypoint function %p...\n", (void*)game_entry.entrypoint);
                fflush(stdout);

                try {
                    game_entry.entrypoint(rdram, context);
                    fprintf(stdout, "[Conker] game_entry.entrypoint returned normally.\n");
                    fflush(stdout);
                } catch (ultramodern::thread_terminated& terminated) {
                    fprintf(stdout, "[Conker] game_entry.entrypoint yielded (thread_terminated).\n");
                    fflush(stdout);
                }
            }
            return true;
