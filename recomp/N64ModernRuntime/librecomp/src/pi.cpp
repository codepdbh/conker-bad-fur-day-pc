...
void recomp::do_rom_read(uint8_t* rdram, gpr ram_address, uint32_t physical_addr, size_t num_bytes) {
    if (rom.empty() || num_bytes == 0) return;
    size_t rom_offset = (physical_addr >= recomp::rom_base) ? (physical_addr - recomp::rom_base) : physical_addr;
    rom_offset %= rom.size();
    if (rom_offset + num_bytes > rom.size()) {
        num_bytes = rom.size() - rom_offset;
    }
    uint32_t dest_phys = ((uint32_t)ram_address) & 0x00FFFFFF;
    if (dest_phys + num_bytes <= 0x01000000) {
        std::memcpy(rdram + dest_phys, rom.data() + rom_offset, num_bytes);
    }
}
